  ...
  scanDb(contacto)
    .then(resultDb => {
      if (Object.keys(resultDb).length != 0) {
        ...
        const newCliente = {
          id: idGenerado, // Ahora se genera un id al azar
          contacto: contacto, // El contacto sigue siendo parte del cliente
          nombre: nombre,
          password: password,
          activo: true,
          registrado: true,
...
/** Función para escanear la tabla de tickets */
async function scanDBTickets() {
    try {
        const input = { // ScanInput
            ...
            Select: "SPECIFIC_ATTRIBUTES",
            AttributesToGet: [
                'id', 'descripcion', 'estado', 'fecha_creacion', 'cliente_id'
            ],
        };

        const response = await docClient.scan(input).promise().then((data) => {
            return data.Items;
        });
